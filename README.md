...
| Services and stack | Entra Private Access; Virtual Machines; Private DNS; Key Vault; Log Analytics |
...
**Recommended view: Zero-trust private-application access path.** A topology view is the strongest complement because it makes network and identity boundaries, enforcement hops, and the governed path physically legible.

![Icon-based zero-trust private-application access path for Entra-Private-Access-Bootstraper](docs/operational-view.svg)

The view follows **Authenticate user and device → Evaluate access policy → Traverse private connector → Reach approved application**. Use it during design reviews, operational walkthroughs, and failure-mode discussions; use the logical architecture above when the question is which technologies integrate.
